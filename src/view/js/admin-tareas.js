...
//listar tareas
function actualizarPaginacion2(paginacion, contenedorId) {
    const { pagina_actual, total_paginas } = paginacion;
    const controles = document.getElementById(contenedorId);
    controles.innerHTML = ''; // Limpiar controles antiguos
...
async function listarTareas(pagina = 1) { // Acepta el número de página
    try {
        const datos = new FormData();
        datos.append('sesion', session_session);
        datos.append('token', token_token);
        ...
        let respuesta = await fetch(base_url + 'src/control/Tarea.php?tipo=listarTareasPaginado', {
            method: 'POST',
            mode: 'cors',
            cache: 'no-cache',
            body: datos
        ...
        let json = await respuesta.json();
        const tbody = document.querySelector('#tbody_tbl_verTareas');
        tbody.innerHTML = ''; // Limpiar tabla antes de agregar nuevas filas
        ...
        if (json.status && json.contenido.length > 0) {
            ...
            // <-- AÑADIDO: Llamar a la función para construir la paginación
            actualizarPaginacion2(json.paginacion, 'pageees-control');

        } else {
             tbody.innerHTML = '<tr><td colspan="9" class="text-center">No hay personas para mostrar.</td></tr>';
             // Limpiar paginación si no hay resultados
             document.getElementById('pageees-control').innerHTML = '';
        }

    } catch (e) {
        console.log("Oops salió un error: " + e);
        document.querySelector('#tbody_tbl_verTareas').innerHTML = '<tr><td colspan="9" class="text-center text-danger">Error al cargar los datos.</td></tr>';
    }
}

// --- AÑADIDO: Event Listener para los clics en la paginación ---
document.getElementById('pageees-control').addEventListener('click', function(e) {
    e.preventDefault(); // Prevenir la recarga de la página
...
async function eliminarTareaa(id_tarea) {
            try {
                let datos = new FormData();
                datos.append('sesion', session_session);
                datos.append('token', token_token);
                datos.append('id_tarea', id_tarea);
                let respuesta = await fetch(base_url + 'src/control/Tarea.php?tipo=eliminarTarea', {
                    method: 'POST',
                    mode: 'cors',
                    cache: 'no-cache',
                    body: datos
